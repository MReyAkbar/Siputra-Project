...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-600">Masukkan data customer baru</p>
        </div>
  ...
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
      <div class="p-8 bg-gradient-to-br from-[#134686] to-[#0d3566]" x-data="createCustomerForm()">
        <form @submit.prevent="submitForm" novalidate>
          ...
          <div class="mb-6">
            ...
            <input 
              ...
              :class="{'ring-2 ring-red-500': errors.includes('Nama customer wajib diisi.')}"
            >
          </div>
          ...
          <div class="flex justify-end gap-4">
            ...
            >
              ...
              <span x-text="loading ? 'Menyimpan...' : 'Simpan Customer'"></span>
            </button>
          </div>
        </form>
      </div>
...
function createCustomerForm() {
  return {
    ...
    form: {
      nama_customer: "{{ old('nama_customer') }}",
      no_hp: "{{ old('no_hp') }}",
      alamat: "{{ old('alamat') }}"
    },
    ...
    async submitForm() {
      ...
      try {
        const response = await fetch("{{ route('manajemen.pengguna.manage-customer.store') }}", {
          method: 'POST',
          body: formData,
          headers: {
