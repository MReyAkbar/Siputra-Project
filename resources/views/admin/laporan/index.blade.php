    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-600">Ringkasan transaksi harian ikan</p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <div x-data="{ open: false }" class="relative">
              ...
              <div x-show="open" x-transition @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                <a href="{{ url('admin/laporan/harian') }}" class="block px-4 py-3 text-sm font-medium bg-blue-50 text-blue-700">Harian</a>
                <a href="{{ url('admin/laporan/mingguan') }}" class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-100">Mingguan</a>
                <a href="{{ url('admin/laporan/bulanan') }}" class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-100">Bulanan</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200" id="laporan-table">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            @php
              $data = [
                ['2025-11-09', 'IKN001', 'Ikan Tuna', 'Pembelian', 280],
                ['2025-11-04', 'IKN002', 'Ikan Barracuda', 'Penjualan', 195],
                ['2025-11-02', 'IKN003', 'Ikan Layang', 'Pembelian', 310],
                ['2025-10-24', 'IKN004', 'Ikan Kakap Merah', 'Penjualan', 120],
                ['2025-10-17', 'IKN005', 'Ikan Kembung', 'Pembelian', 450],
              ];
            @endphp
            ...
            <tr class="hover:bg-gray-50 transition">
              ...
              <td class="px-6 py-4"><span class="px-3 py-1 text-xs font-semibold rounded-full {{ $row[3] == 'Pembelian' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800' }}">{{ $row[3] }}</span></td>
      ...
      <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex items-center justify-between">
        <div class="text-sm text-gray-700">
          Menampilkan <span class="font-medium">1</span> sampai <span class="font-medium">10</span> dari <span class="font-medium">5</span> hasil
        </div>
...
function exportToExcel() {
    ...
    XLSX.writeFile(wb, `Laporan_Harian_SIPUTRA_${new Date().toISOString().slice(0,10)}.xlsx`);
}
</script>
@push('scripts')
