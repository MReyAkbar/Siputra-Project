...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br px-4">
    <div class="w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <p class="text-gray-400">Daftar untuk mengakses lebih banyak fitur SIPUTRA</p>
        </div>

        <form method="POST" action="{{ route('register') }}" class="bg-white rounded-xl shadow-lg p-8 space-y-6">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Nama Lengkap</label>
                <input type="text" name="name" value="{{ old('name') }}"
                       class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#134686] focus:border-transparent @error('name') border-red-500 @enderror"
            ...
            <p class="text-center text-sm text-gray-600">
                Sudah punya akun?
                <a href="{{ route('login') }}" class="text-[#134686] font-medium hover:underline">Masuk di sini</a>
            </p>
        </form>
    </div>
</div>
