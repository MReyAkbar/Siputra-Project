...
<div class="min-h-screen flex items-center justify-center bg-gradient-to-br  px-4">
    <div class="w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <p class="text-gray-400">Masuk ke akun SIPUTRA Anda</p>
        </div>

        <form method="POST" action="{{ route('login') }}" class="bg-white rounded-xl shadow-lg p-8 space-y-6">
            @csrf
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center">
                    <input type="checkbox" name="remember" class="w-4 h-4 text-[#134686] rounded">
                    <span class="ml-2 text-sm text-gray-600">Ingat saya</span>
                </label>
                <a href="{{ route('password.request') }}" class="text-sm text-[#134686] hover:underline">
                    Lupa password?
                </a>
            </div>
            ...
            <p class="text-center text-sm text-gray-600">
                Belum punya akun?
                <a href="{{ route('register') }}" class="text-[#134686] font-medium hover:underline">Buat akun</a>
            </p>
        </form>
    </div>
</div>
