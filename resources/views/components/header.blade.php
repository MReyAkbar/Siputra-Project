  ...
  <div class="max-w-7xl mx-auto flex items-center justify-between p-4">
    ...
    <div class="hidden md:flex items-center space-x-4">
      @guest
        ...
        <a href="{{ route('login') }}" class="px-5 py-2 bg-[#0C3C65] rounded-xl font-medium hover:bg-white hover:text-[#134686] transition">Log In</a>
      @else
        <div class="flex items-center gap-3">
          <span class="text-sm font-medium">Hai, {{ auth()->user()->name }}</span>
          <form method="POST" action="{{ route('logout') }}" class="inline">
            @csrf
            <button type="submit" class="px-4 py-2 bg-[#0C3C65] rounded-lg text-sm font-medium hover:bg-white hover:text-[#134686] transition">Logout</button>
          </form>
        </div>
      @endguest
    </div>
